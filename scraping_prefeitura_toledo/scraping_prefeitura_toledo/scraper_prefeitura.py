...
SELENIUM_BROWSER = os.getenv("SELENIUM_BROWSER", "edge").strip().lower()
...
def separar_processo_exercicio(numero_protocolo):
    ...
    original = str(numero_protocolo).strip()
    texto = re.sub(r"\s+", "", original)

    if re.fullmatch(r"\d{2}\.?\d{3}\.?\d{3}/\d{4}-\d{2}", texto):
        raise ValueError(
            f"O valor parece ser um CNPJ, nao um numero de processo: {numero_protocolo}."
        )
    ...
    if not ano_match:
        raise ValueError(
            ...
            "Use algo como 49428/2023, 59.226/2024 ou 073-001351/2025."
        )

    exercicio = ano_match.group(1)
    antes_do_ano = texto[: ano_match.start()]

    grupos_numericos = re.findall(r"\d[\d.]*", antes_do_ano)
    if not grupos_numericos:
        raise ValueError(f"Nao encontrei o numero do processo em: {numero_protocolo}")

    processo = grupos_numericos[-1].replace(".", "")

    if not processo or len(processo) > 7:
        raise ValueError(
            f"Numero de processo invalido extraido de '{original}': '{processo}'."
        )

    return processo, exercicio


def configurar_options(options, headless=False):
    if headless:
...
def criar_driver(headless=False):
    if SELENIUM_BROWSER in ["chrome", "chromium"]:
        options = configurar_options(ChromeOptions(), headless=headless)
    ...
    options = configurar_options(EdgeOptions(), headless=headless)
...
def extrair_mensagens_notificacao(driver):
    mensagens = []
    seletores = [
        "//*[@data-test='text-message']",
        "//*[contains(concat(' ', normalize-space(@class), ' '), ' notification ')]",
...
def esperar_resultado_ou_erro(driver, wait):
    def resultado_ou_erro(d):
        mensagens = extrair_mensagens_notificacao(d)
        if mensagens:
            return {"erro": " | ".join(mensagens)}

        texto = normalizar_texto(d.find_element(By.TAG_NAME, "body").text)
        if "situacao do processo" in texto or "data de ocorrencia" in texto:
            return {"ok": True}

        return False

    retorno = wait.until(resultado_ou_erro)
    if retorno.get("erro"):
...
def consultar_prefeitura(protocolo, headless=False):
    ...
    try:
        ...
        esperar_resultado_ou_erro(driver, wait)
        try:
